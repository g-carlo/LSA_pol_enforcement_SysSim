...
noise_thresh = 3e-3; % 5dBm

% Power level 
LU_power = 0.72;  % 43 dBm over 5MHz so (180/5000)*20 W = 0.72 W per channel
IU_power = 9e-4;  % 20 dBm over 20MHz so (180/20000)*0.1 W = 9e-4 W per channel
...
for t=1:T
    ...
    ratio_ols = norm(x_star(1:Cells))^2/norm(x_star(1:Cells) - x_ols(1:Cells))^2;
    ...
    ratio_stela = norm(x_star(1:Cells))^2/norm(x_star(1:Cells) - x_stela(1:Cells))^2;
...
%      Rat_wstela = Rat_wstela + ratio_wstela;
    ...
    ratio_irwl = norm(x_star(1:Cells))^2/norm(x_star(1:Cells) - x_irwl(1:Cells))^2;
...
%     sup_stela2 = find(Av_En_stela>noise_thresh);
   ...
   % sup_irwl = find(Av_En_irwl>sig_n^2);    
    ...
    % Error    
    E_star(Cells+1:1:Cells+N_IUs) = 0;
    Av_r_err_ols(i)= norm(E_star - Av_En_ols,2)/norm(E_star,2);
    Av_r_err_stela(i)= norm(E_star - Av_En_stela,2)/norm(E_star,2);
    Av_r_err_irwl(i)= norm(E_star - Av_En_irwl,2)/norm(E_star,2);
 
%     if Av_r_err_stela(i)>1
%         
%         break;
%         
%     end
   ...
   % Soft decision probability OLS
    ...
    Sdc_ls = setdiff(L(1:LU_Transimtters),Sup_ls);
    Sfa_ls = setdiff(Sup_ls,L(1:LU_Transimtters));
    ...
    S2dc = setdiff(L(1:LU_Transimtters),Sup_STELA);
    S2fa = setdiff(Sup_STELA,L(1:LU_Transimtters));
   ...
   %%%%%%%%%%%%%%%%%%%%%%  5. IRWL-LASSO prob.%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    Sdc_irwl = setdiff(L(1:LU_Transimtters),Sup_irwl);
    Sfa_irwl = setdiff(Sup_irwl,L(1:LU_Transimtters));
...
%  close(hb);
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%% 1. OLS computations %%%%%%%%%%%%%%%%%%%%%%%%%%%
  Time_ols = mean(MT1);
  Relative_error_ols = sum(Av_r_err_ols)*100/It;
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%% 2. STELA computations %%%%%%%%%%%%%%%%%%%%%%%%%
  Time_stela = mean(MT2);
  Relative_error_stela = sum(Av_r_err_stela)*100/It;
...
%%%%%%%%%%%%%%%%%%%%%%%%%% 5. IRWL-STELA computations %%%%%%%%%%%%%%%%%%%%%%%%%
  Time_irwl = mean(MT5);
  Relative_error_irwl = sum(Av_r_err_irwl)*100/It;
