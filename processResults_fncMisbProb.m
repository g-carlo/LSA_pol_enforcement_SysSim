function fig3=processResults_fncMisbProb(n_samples,filelabelRes,Thresholds)
% process results
if nargin==2
    Thresholds = [];
end
...
if isGain 
    ...
    fig3 = figure(3);
    k = ceil(n_samples*3/50);
    plot(tsmovavg(av_spectrum_vector(1,:),'t',k),'LineWidth',2);
    hold on;
    plot(tsmovavg(av_spectrum_vector(2,:),'t',k),'LineWidth',2);
    plot(tsmovavg(av_spectrum_vector(3,:),'t',k),'LineWidth',2);
    hold off;
    grid on;
    ylim([0 0.8])
    ...
    legend('Location','northwest')
    title('LSA SLS Resource Allocation')
