function processResults_fnc(n_samples,filelabelRes,Thresholds)
% process results
...
%     axis([1 n_samples 0 1])
    ...
    fig3 = figure(3);
    plot(tsmovavg(av_spectrum_vector(1,:),'t',30));
    hold on;
    plot(tsmovavg(av_spectrum_vector(2,:),'t',30));
%     hold on;
    plot(tsmovavg(av_spectrum_vector(3,:),'t',30));
    axis([1 n_samples 0 1])

end

    % filename_data = [filelabelRes '.mat'];

    filename = ['Res_' filelabelRes];
    save(filename,'n_samples','av_spectrum_vector');
    savefig(fig1,[filename '_1']);
    savefig(fig2,[filename '_2']);
