function conf = getInputParameters_MBEH(Misbehaving_probs)
...
%     conf.carrier.numOfAvailablePRB            = [50 50 50 50];                   % number of available PRBs
    ...
    conf.general.numOfTTIperSimulation        = 10;     % simulation length in TTIs
    conf.general.simulatedTime                = 6*24*30;     % simulation length in TTIs   
...
%     conf.operator.misBehProbability             = [0.8 0.01 0.0];       
    conf.operator.misBehProbability             = Misbehaving_probs;       
    conf.operator.cellSpectralEfficiency        = 2.23;  
    
    conf.L1_scheduling.schedulingSlotLength     = 24;      
    conf.L1_scheduling.penaltyLambda            = 240/0.69;          % with X/0.69, penalty halves at X     
